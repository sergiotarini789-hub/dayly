...
import { Badge, Button, Input, Progress, RadioGroup, RadioItem, Select } from "@/components/ui";
...
export function OnboardingExperience() {
  ...
  return (
    <main className="dayly-onboarding" aria-labelledby="onboarding-heading">
      <div className="dayly-onboarding__inner">
        ...
        <div className="dayly-onboarding__intro">
          <Badge variant="primary">Personal setup</Badge>
          <p className="dayly-product-eyebrow">A lighter start</p>
        ...
        <div className="dayly-onboarding__progress" aria-label={`Onboarding step ${step + 1} of ${steps.length}`}>
          ...
          <ol>{steps.map((label, index) => <li key={label} data-current={index === step || undefined} aria-current={index === step ? "step" : undefined}>{label}</li>)}</ol>
        </div>
